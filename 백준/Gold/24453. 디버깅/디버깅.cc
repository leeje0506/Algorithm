...
void solution() {
    ...
    int ans=INT_MAX; // ans는 인규가 지워야하는 최소 에러 개수
    ...
    while(1){
        ...
        if((er_cnt<Y||su_cnt<X)&&end<=N){ //모든 조건을 만족하지 않은 경우 end를 움직인다.
            if(error[end++]) er_cnt++; //현재 줄에 에러가 있다면 에러 개수를 늘려
            su_cnt++;
        }else{
            if(er_cnt>=Y&&su_cnt>=X) ans=min(ans,er_cnt); //모든 조건을 만족한다면 ans를 갱신해준다.
            if(error[start++]) er_cnt--; //현재 줄에 에러가 있다면 에러 개수를 줄여
            su_cnt--;
        }
    }
    
    //답 : 전체 에러의 개수 - 최소로 인규가 지워야하는 에러의 개수
    cout<<M-ans;
}
